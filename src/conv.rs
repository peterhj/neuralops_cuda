...
impl<S> DeviceConv2dNoBiasOperator<S> {
  pub fn new<InOp>(cfg: Conv2dOperatorConfig, cap: OpCapability, prev_op: Rc<RefCell<InOp>>, prev_arm: usize, stream: DeviceStream) -> Rc<RefCell<DeviceConv2dNoBiasOperator<S>>> where InOp: 'static + DeviceOperator + NewDiffOperator<S, IoBuf=[f32]> {
    let (in_w, in_h, in_chan) = cfg.in_dim;
    let (out_w, out_h, out_chan) = cfg.out_dim();
    let mut workspace_size = 0;
    let add_bias = CudnnAddOp::new(
    ...
    workspace_size = max(workspace_size, bwd_d.work_size);
    let in_ = prev_op.borrow()._output(prev_arm);
    Rc::new(RefCell::new(DeviceConv2dNoBiasOperator{
      cfg:      cfg,
      node:     OperatorNode::default(),
      stream:   stream.clone(),
      in_op:    prev_op,
...
impl<S> Operator for DeviceConv2dNoBiasOperator<S> {
  fn _next(&self) -> u64 {
    self.node._next()
  }
...
impl<S> DeviceOperator for DeviceConv2dNoBiasOperator<S> {
  fn _output(&self, arm: usize) -> DeviceOutput {
    assert_eq!(0, arm);
    self.out.clone()
  }
...
impl<S> NewDiffOperator<S> for DeviceConv2dNoBiasOperator<S> {
  type IoBuf = [f32];
