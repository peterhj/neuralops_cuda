...
pub fn build_cifar10_affine_kl_loss<IoBuf: ?Sized + 'static>(batch_sz: usize, stream: DeviceStream) -> Rc<RefCell<DeviceSoftmaxKLLoss<SampleItem, IoBuf>>> {
  let input_cfg = VarInputOperatorConfig{
    batch_sz:   batch_sz,
    max_stride: 32 * 32 * 3,
    out_dim:    (32, 32, 3),
  ...
  let affine1_cfg = AffineOperatorConfig{
    batch_sz:   batch_sz,
    in_dim:     32 * 32 * 3,
    out_dim:    256,
    bias:       false,
  ...
  let affine2_cfg = AffineOperatorConfig{
    batch_sz:   batch_sz,
    in_dim:     256,
    out_dim:    10,
    bias:       false,
  ...
  let affine2 = DeviceAffineOperator::new(affine2_cfg, OpCapability::Backward, affine1, 0, stream.clone());
  let loss = DeviceSoftmaxKLLoss::new(loss_cfg, OpCapability::Backward, affine2, 0, stream.clone());
  loss
}

pub fn build_cifar10_simpleconv_kl_loss<IoBuf: ?Sized + 'static>(batch_sz: usize, stream: DeviceStream) -> Rc<RefCell<DeviceSoftmaxKLLoss<SampleItem, IoBuf>>> {
  let input_cfg = VarInputOperatorConfig{
    batch_sz:   batch_sz,
    max_stride: 32 * 32 * 3,
    out_dim:    (32, 32, 3),
  ...
  let conv4 = DeviceConv2dOperator::new(conv4_cfg, OpCapability::Backward, conv3, 0, stream.clone());
  let affine = DeviceAffineOperator::new(affine_cfg, OpCapability::Backward, conv4, 0, stream.clone());
  let loss = DeviceSoftmaxKLLoss::new(loss_cfg, OpCapability::Backward, affine, 0, stream.clone());
  loss
}
